# Unitree H2 Sonic MuJoCo Joint Order

`h2_sonic_mujoco_joint_order.json` records the order used by this repository
when saving retargeted Sonic H2 motion:
...
- `actuator_index` is the MuJoCo actuator/control index in `h2.xml`; actuator
  names omit the `_joint` suffix in the Sonic training MJCF.
